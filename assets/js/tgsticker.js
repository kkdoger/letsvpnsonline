var RLottie = (function () {
  ...
  function initApi(callback) {
    if (apiInited) {
      ...
      if (!apiInitStarted) {
        ...
        apiInitStarted = true;
        QueryableWorkerProxy.init(
          '/js/tgsticker-worker.js?14',
          rlottie.WORKERS_LIMIT,
          function () {
            apiInited = true;
  ...
  function initPlayer(el, options) {
    ...
    var tgs_sources = el.querySelectorAll(
    ...
    for (var i = 0; i < tgs_sources.length; i++) {
      ...
      var frames_align =
        (tgs_source && tgs_source.getAttribute('data-frames-align')) || '';
      if (url && !urls_map[url]) {
        urls_map[url] = true;
        urls.push({
          url: url,
    ...
    var curDeviceRatio = options.maxDeviceRatio
      ? Math.min(options.maxDeviceRatio, deviceRatio)
      : deviceRatio;
    ...
    rlPlayer.workerProxy = QueryableWorkerProxy.create(
      rlPlayer.reqId,
      onFrame,
      onLoaded
    );
  ...
  function render(rlPlayer, checkViewport) {
    ...
    if (frame !== null) {
      ...
      if (rlPlayer.stopOnFirstFrame && frame.no == 0) {
    ...
    if (
      ...
      (!frameNo ||
        (rlPlayer.options.cachingModulo &&
          (reqId + frameNo) % rlPlayer.options.cachingModulo))
    ) {
  ...
  function onLoaded(reqId, frameCount, fps) {
    ...
    if (frameCount > 0) {
      if (rlPlayer.needPlayOnce) {
        ...
        rlPlayer.stopOnLastFrame = false;
        if (rlPlayer.frameNo === false || rlPlayer.frameNo > 0) {
...
var QueryableWorkerProxy = (function () {
    ...
    if (finished) {
      if (proxy.items.length == 1) {
        ...
        Promise.all(promises).then(function (bitmaps) {
          ...
          var imageData = proxy.context.getImageData(
            0,
            0,
            proxy.canvas.width,
            proxy.canvas.height
  ...
  function onLoaded(wReqId, frameCount, fps) {
    ...
    if (finished) {
      if (frameCount === null) {
        console.warn('Frame count not defined');
        return;
      }
      if (frameCount === false) {
        console.warn('Frame count is different');
        return;
      }
      if (fps === null) {
        console.warn('FPS not defined');
        return;
      }
      if (fps === false) {
        console.warn('FPS is different');
        return;
      }
    ...
    firstWorker.addListener('ready', function () {
      ...
      if (!workersRemain) {
        ...
        for (var workerNum = 1; workerNum < workers_limit; workerNum++) {
          (function (workerNum) {
            var rlottieWorker = (rlottieWorkers[workerNum] =
              new QueryableWorker(worker_url));
