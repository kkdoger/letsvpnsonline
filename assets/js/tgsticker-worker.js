...
RLottieItem.prototype.init = function (jsString) {
  try {
    ...
    this.frameCount = RLottieWorker.Api.loadFromData(
      this.handle,
...
RLottieItem.prototype.render = function (frameNo, clamped) {
  ...
  try {
    ...
    var data = Module.HEAPU8.subarray(
      bufferPointer,
      bufferPointer + this.width * this.height * 4
    );
...
function reply() {
  ...
  if (isSafari(self)) {
    ...
    postMessage(
      { queryMethodListener: arguments[0], queryMethodArguments: args },
      transfer
    );
  }
}
    ...
    xhr.onreadystatechange = function (event) {
      if (xhr.readyState === 4) {
        if (xhr.status === 200 || xhr.status === 0) {
          ...
          callback(
            new Error('Ajax error: ' + this.status + ' ' + this.statusText)
          );
        }
      }
    };
