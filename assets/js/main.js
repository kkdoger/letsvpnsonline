...
function blogSideImageUpdate(argument) {
  ...
  document
    .querySelectorAll('.blog_side_image_wrap')
    .forEach(function (imageWrap) {
      if (isDesktop) {
        var titleHeight =
          imageWrap.parentNode.previousElementSibling.clientHeight;
        var beforeTitleEl =
          imageWrap.parentNode.previousElementSibling.previousElementSibling;
        if (beforeTitleEl) {
...
function backToTopInit(labelHtml) {
  ...
  t.innerHTML =
    '<div class="back_to_top"><i class="icon icon-to-top"></i>' +
    labelHtml +
    '</div>';
  toTopEl = t.firstChild;
  t.innerHTML =
    '<a class="back_to_top_wrap' +
    (pageContentWrapEl.classList.contains('is_rtl') ? ' is_rtl' : '') +
    '" onclick="backToTopGo()"></a>';
...
function getDevPageNav() {
  ...
  $('a.anchor').each(function (k, anchor) {
    ...
    if (level == 3) {
      ...
      var subMenu =
        $('ul', lastLi)[0] || $('<ul class="nav"></ul>').appendTo(lastLi);
      // console.log(subMenu);
      li.appendTo(subMenu);
    }
    items++;
...
function initDevPageNav() {
  ...
  var sideNavWrap = $('<div class="dev_side_nav_wrap"></div>').prependTo(
    sideNavCont
  );
...
function initScrollVideos(desktop) {
  ...
  for (var i = 0; i < pageVideos.length; i++) {
    ...
    var preloadValue =
      i >= preloadVideos ? (isSafari ? 'none' : 'metadata') : 'auto';
    ...
    if (posterUrl && isSafari) {
      videoEl.parentNode.style.background =
        "url('" + escapeHTML(posterUrl) + "') center no-repeat";
...
function initRipple() {
  ...
  var rippleTextFields = document.querySelectorAll(
  ...
  for (var i = 0; i < rippleTextFields.length; i++) {
    (function (rippleTextField) {
      function onTextRippleStart(e) {
        ...
        var ripple = rippleTextField.parentNode.querySelector(
          '.textfield-item-underline'
        );
        var rippleX =
          ((clientX - rect.left) / rippleTextField.offsetWidth) * 100;
